...
Multilingual evaluations report accuracy at a single output-token cap, but languages need different numbers of tokens to express the same content, which makes that cap a hidden experimental variable. We test whether the native-vs-translate gap on MGSM (German, Thai, Swahili) is a token-budget artifact, sweeping the budget rather than fixing it, for Qwen3-8B and Llama-3.1-8B-Instruct under four prompting strategies. The measured gap moves by up to 39 points across budgets, and at tight caps length normalization can reverse which strategy scores higher. We pre-registered the sweep's three Qwen peaks and its near-zero value at 1024, then tested them on 540,000 independently hard-capped decodes; a second frozen family of six Holm-corrected tests rejects every null. The prospectively frozen test at \(B^*=1024\) still fails to reject, because native accuracy has already saturated there; above saturation the residual difference is a strategy-performance gap, not an identified reasoning deficit. The same truncation channel prices a cost-ordered adaptation ladder: a cross-fitted Thai vocabulary extension closes 0.00 points of the gap at the frozen budget and 4.9 points where 19% of traces still truncate. A third frozen family varies only the *announced* budget at a fixed enforced cap, and announcing 128 rather than 2048 tokens moves Thai native accuracy by 5.1 points, so accuracy is not a function of the enforced cap alone. Treat the output cap as an independent variable and report accuracy across the budget regime, not at a single budget.
...
Independent decoding. Because prefix budgets share a trajectory, we regenerated the sweep with decodes that were hard-capped one budget at a time. The stored ledger becomes a discovery sample: its published sweep values were pre-registered as point predictions in `prereg-independent-decoding.md`, frozen at git tag `independent-protocol-freeze` before any of the new generation ran, and then tested on a fresh confirmation sample. The peak budgets under test were fixed from discovery and not re-selected. We generated 540,000 decodes - 270 cap-partitioned shards of 2,000 records each, covering both models, all four arms, three languages, and \(k=8\) - over \(B \in \{64,128,192,256,384,512,768,1024,2048\}\) together with the premium-scaled caps \(\lfloor r_{m,L}B\rfloor\) for NATIVE. The seed varies with the cap, so no two budgets replay one trajectory. We verified all 270 shards; no trace exceeds its cap.
...
**The peaks survive independent capping.** The confirmation sample carries its own frozen family of six Holm-corrected Qwen tests: three one-sided SESOI tests that \(\Delta_L(B)>5\) points at the discovery peak budget, and three two-one-sided equivalence tests that \(|\Delta_L(1024)|<5\). All six reject at family-wise \(\alpha=0.05\) (formal outcome `confirmatory_support`). The independent peak estimates are 34.65 points for German at \(B=192\), 38.60 for Thai at 256, and 13.70 for Swahili at 128, with standard errors 2.10, 2.26, and 1.37 and \(p=0.0001\) throughout. Each falls inside the published discovery interval. The equivalence estimates at \(B^*=1024\) are 0.15, -0.25, and -1.25 points, the largest of their three p-values being 0.0003. Peak location replicates as well: the independent argmax sits on the predicted budget in all three Qwen languages. This is a second frozen family rather than a re-run of the first. The \(B^*=1024\) family of §3.1 still fails to reject, and that result is unchanged. What the replication rules out is that the tight-budget magnitudes were an artifact of scoring one stored trajectory at many cut points.

Llama remains outside the confirmatory family and supports no confirmatory claim. Its independent estimates at the discovery peaks are 8.50 points for German, 2.10 for Thai, and 17.65 for Swahili, against discovery values of 8.35, 2.30, and 18.20. The Thai SESOI test does not reject, which the protocol predicted before any scoring: a discovery estimate of 2.30 points cannot clear a 5-point SESOI. Llama German and Thai each shift their argmax by one grid point, and the protocol had likewise flagged those cells as too flat to fix a location - Thai reads 2.20, 2.30, and 2.00 at 128, 192, and 256. A length check supports reading the two frames as one process: independently decoded output-length distributions match the truncated discovery distributions to a median 0.11% of the cap for Qwen and 0.16% for Llama.
...
**Instrument validity.** A null is interpretable only once the manipulation is shown to have arrived, and §3.4 has its own near-miss to show for it. The announcement protocol gated every cell on a 30% median-length reduction between announced budgets - the gate that had already removed Swahili from the family - and both TRANSLATE-ACT cells failed it, at 14.6% for German and 10.1% for Thai. Their nulls were uninformative rather than negative. A diagnostic located the cause: under the shipped sentence the translation segment did not respond at all, holding at 57 tokens in German and 76 in Thai whichever budget was announced. A stronger sentence reaches 34.4% and 37.5% and passes the gate, and under it the two estimates are \(-2.60\) (\(p=0.0229\)) and \(-2.30\) (\(p=0.0662\)). No Holm decision changes and the formal outcome is unchanged; what changes is that the two nulls become real nulls. We report both instruments (Appendix H). Llama fails the gate in all four cells under both instruments, at 2.4-9.3% against Qwen's 34-43%, so every Llama estimate there is uninformative about budget sensitivity, including the two that reject. Llama carries no confirmatory claim, so nothing in the family depends on it.
...
Limitations. (i) The tight-budget magnitudes no longer rest on prefixes of one long decode: the three Qwen peak cells and the equivalence at \(B^*\) were pre-registered and confirmed on independently capped generations (§3.2). The rest of the sweep is still retrospective and exploratory - the crossovers in §3.3, the grid points outside those six cells, and the normalizer-sensitivity analysis. That replication removes the shared-trajectory artifact without testing budget-aware generation. Under our serving stack `max_tokens` only stops decoding and never conditions the model: with a shared seed, 75% of capped decodes come back bitwise identical to the truncated long decode. Announcement is a separate channel, and §3.4 tests it directly: with the enforced cap held fixed, telling the model its budget changes accuracy in one of four frozen cells. What remains untested is the interaction - whether an announced budget that also binds behaves like either frame alone - and whether announcement effects hold outside Qwen, MGSM, and the announced values under test. The forcing results are exploratory. (ii) vLLM repeat generation was only 46% bitwise deterministic, preserving the stored-ledger estimand but weakening reproducibility and shared-seed interpretation. (iii) Scope is MGSM, three languages, and two 8B models. (iv) The frozen human GlotLID validation remains outstanding, despite the preliminary blind LLM agreement. (v) The frozen same-content trace-premium validation was not performed; the behavioral ratio in Appendix C does not substitute for it. (vi) Calibration is only approximately nominal (corrected type-I 0.00917 vs 0.00833 target); the 1.3x factor is a family-wide safeguard, not a verified family-wise calibration. (vii) The vocabulary-extension results in §5 are a token-count-only counterfactual. Prefixes are rescored under an extended tokenizer while the emitted text is held fixed, so they do not predict a retrained model, whose trajectory would differ and whose new embeddings must themselves be learned. They cover Qwen only, because the extension is tokenizer-specific. The extensions are trained on MGSM reasoning traces rather than general text, so the observed compression is a property of this corpus, not a bound on what is achievable. The item-clustered bootstrap holds the two cross-fitted tokenizers fixed, so intervals are conditional on that draw, and the fold split is not randomized. Both arms are scored by retokenizing the stored text, which agrees with replaying stored token ids on 59,998 of 60,000 sample-budget comparisons. \(G(4096)\) is the gap at the largest stored prefix, not a demonstrated non-binding regime.
...
All cells decode at a real cap of 2048 tokens; only the announced number varies. Protocol `prereg-budget-aware.md`, frozen at git tag `budget-aware-protocol-freeze` before any E2 record existed: 876,000 records over 438 shards. The estimand is \(\Delta_{\mathrm{ann}} = \operatorname{acc}^{128}(2048)-\operatorname{acc}^{2048}(2048)\), two-sided, with the same 1.3x tail-conservatism factor used elsewhere in the paper. The confirmatory family is four two-sided contrasts between announced 128 and announced 2048 on Qwen3-8B, Holm-corrected at family-wise \(\alpha=0.05\) (first-step local \(\alpha_1=0.0125\)); the announced-256 cell is an interpolation and sits outside the family by design. Formal outcome: `announcement_effect_detected`.
...
E2b re-runs the four Qwen confirmatory cells under a second announcement sentence. Protocol `prereg-e2b.md`, frozen at git tag `e2b-protocol-freeze`; the family, estimand, announced values, enforced cap and Holm structure are unchanged from Appendix G, and only the TRANSLATE-ACT instruction changed. Instrument v0 reads "The translation, all of your reasoning and the final answer may take at most {budget} tokens in total." Instrument v1 reads "Your entire response must not exceed {budget} tokens. Keep the translation as short as possible, reason concisely, and write the #### line before you reach the limit." NATIVE is unchanged and its records are the same under both instruments, so its rows below are one measurement printed twice rather than a replication. The gate is a 30% median-length reduction between the extreme announced values; a cell that fails the gate yields an uninformative estimate, not evidence of no effect.
