...
1. The answer can depend strongly on the evaluation budget. We pair a prospectively frozen non-rejection at one budget with a retrospective sweep that localizes the budget-binding regime. We then replicate that budget dependence under a pre-registered held-out design in which every budget is decoded under its own hard cap.
