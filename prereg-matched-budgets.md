...
- **Tail-conservatism correction (added 2026-07-24, pre-freeze).** The deposited calibration simulation revealed that the bootstrap sup-t inversion, while asymptotically exact, is mildly **anti-conservative in the extreme tail** used by the tightest Holm-local level (α/6 ≈ 0.0083): measured type-I ≈ 0.0095–0.010 vs the 0.00833 target (~1.15–1.2×) across ~18k calibration sims. Diagnosis (deposited): this is inherent finite-sample behavior of the max-statistic bootstrap with only 250 discrete binary item clusters, **not** a standard-error-estimation artifact — a fixed-SE vs per-replicate (studentized bootstrap-t) SE comparison gave the same rate, so a double-bootstrap SE does not remove it; and at the looser α = 0.05 the same machinery is conservative (0.032 vs 0.05), confirming a tail-only miss. Correction: every confirmatory p-value entering the six-test Holm family (H1-existence, H1-SESOI, H2, H3×3) is multiplied by a **pre-specified tail-conservatism factor of 1.3** (equivalently, tested at the stricter effective level α/1.3), chosen a priori to exceed the measured ~1.2× inflation with margin. The factor is fixed from the measured effect, not tuned per dataset; the deposited calibration re-run **verifies** the corrected type-I is ≤ nominal rather than the factor being adjusted until it passes. Mild power cost only; achieved power at k = 8 (reported in §14) is measured under the corrected test. The null-calibration validation flag is a one-sided upper check (fails only if type-I sits meaningfully above nominal), since a conservative rate is a pass, not a failure.
...
- The bootstrap sup-t test is mildly anti-conservative in the extreme α/6 tail with only 250 discrete clusters; a pre-specified 1.3× tail-conservatism factor (§8) corrects measured type-I to ≤ nominal, at a mild power cost. This is a finite-sample property inherent to the max-statistic bootstrap, not removable by a larger or nested bootstrap; the correction plus the deposited calibration re-run make the confirmatory family's error control verifiable rather than assumed
